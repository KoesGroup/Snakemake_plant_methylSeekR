...
rule BSseeker_build_index:
    ...
    threads: 10
    shell:
        "python BSseeker2/bs_seeker2-build.py -f {input} && touch {output}"
...
rule BSseeker_mapping:
    ...
    run:
        if sample_is_single_end(params.sampleName):
            shell("bs_seeker2-align.py -i {input[0]} -g {input.genome} -o {output.bams}")
        else:
            shell("bs_seeker2-align.py -1 {input[0]} -2 {input[1]} -g {input.genome} -o {output.bams}")
